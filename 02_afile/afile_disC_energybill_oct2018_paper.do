...
global rfile "${inputs}\disC/FINAL_disC_energybill_student_20181022.dta"
global rfile_hhld "${inputs}\disC/FINAL_disC_energybill_hhld_20181022.dta"
global ffile "${outputs}\disC/${pilot}_full_data.dta"
global mfile "${outputs}\disC/${pilot}_model_data.dta"

global startpilot 29oct2018 // date first treatment email was sent out
global endpilot 14jun2019
global startpretreat 01jul2018 
global endpretreat 22oct2018
global useyear 2019


log using "${logs}/disC/disC_energybill_oct2018_afile_log_${today}.log", replace
...
save "${interim}\disC/temp_pilot_${pilot}_data.dta", replace
...
use using "${inputs}\disC\disC_dw_daily_absences" if school_year == $useyear, clear

merge m:1 sid using "${interim}\disC/temp_pilot_${pilot}_data.dta" , gen(_mrgpilotdaily) keepusing(sid treatment exp_grade exp_school)
...
pg_get_weekly , startdate_dmy("$startpilot") enddate_dmy("$endpilot") save("${inputs}/disC/weekly_cumulative_adj_abs.dta")
...
pg_get_weekly , startdate_dmy("$startpretreat") ///
						save("${inputs}/disC/pre_treat_adj_abs.dta") enddate_dmy("$endpretreat")  pre_treat
...
use "${interim}/disC/temp_pilot_${pilot}_data.dta", clear
sum pid

merge 1:1 sid using "${inputs}/disC/weekly_cumulative_adj_abs.dta", gen(_mrgwkly)
...
merge 1:1 sid using "${inputs}/disC/pre_treat_adj_abs.dta", gen(_mrgprewkly)
...
preserve
	...
	use "${inputs}\disC\disC_dw_analysis", clear
...
// mark attrition
gen attrit_post_treat = mi(adj_abs_rate)
